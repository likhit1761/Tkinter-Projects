...
class view:
    ...
    def genera_code(self, frame, SSID, Seguridad, Password, Visibilidad):
        ...
        if SSID != "" and Password != "":
            ...
            qr.add_data(
                'WIFI:T:' + Seguridad + ';S:' + SSID + ';P:' + Password + ';H:' + Visibilidad + ';');  # Url or information that will contain the qr code.
            qr.make(fit=True);
    ...
    def ViewGenerateQR(self):
        ...
        btnGenerar = tk.Button(f2, text="Generar QR",
                               command=lambda: self.genera_code(f, txtSSID.get(), cbxSeg.get(), txtPwd.get(),
                                                                cbxShow.get()));
