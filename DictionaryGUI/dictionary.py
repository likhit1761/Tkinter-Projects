    ...
    def __init__(self, login_root):
        ...
        def login():
            ...
            if ans1 == True and ans2 == True:
                conn = sqlite3.connect('details0.db')
                with conn:
                    cursor = conn.cursor()
                cursor.execute('CREATE TABLE IF NOT EXISTS details0 (name TEXT, email TEXT, password TEXT)')
                with conn:
                    cursor.execute("SELECT email,password FROM details0")
                    ans = cursor.fetchall()
                    for i in ans:
        ...
        def signup_user():
            ...
            if ans:
                ...
                with conn:
                    cursor = conn.cursor()
                cursor.execute('CREATE TABLE IF NOT EXISTS details0 (name TEXT, email TEXT, password TEXT)')
                cursor.execute('INSERT INTO details0 (name,email,password) '
